...
auto Rc2outIS48IE64 = make_vector<uint32_t>(64, 1024);
auto s_a_Rc2outIS48IE64 = make_vector<share*>(64, 1024);
auto IRc2outIS48IE64 = make_vector<uint32_t>(64, 1024);
auto s_a_IRc2outIS48IE64 = make_vector<share*>(64, 1024);
{
ifstream fin("IRc2in");
while(!fin.eof()){
	uint32_t i, j, val;
...
for(int i = 48; i < 64; i++) {
	for(int j = 0; j < 1024; j++) {
		fout << i << " " << j << " " <<(IRc2outIS48IE64[i][j] = rand()) << endl;
	}
}
fout.close();
}
...
for (uint32_t i = 48; i < 64; i++)
{
    ...
    {
        if (role == SERVER) {
            Rc2outIS48IE64[i][j] = IRc2outIS48IE64[i][j] ;
            s_a_Rc2outIS48IE64[i][j] = acirc->PutINGate( Rc2outIS48IE64[i][j] ,bitlen,SERVER);
        } else {
            s_a_Rc2outIS48IE64[i][j] = acirc->PutDummyINGate(bitlen);
        }
    }
}
for (uint32_t i = 48; i < 64; i++)
...
for (uint32_t i = 48; i < 64; i++)
{
    for (uint32_t j = 0; j < 1024; j++)
    {
        ...
        {
            share * s_a_tmp_0 = acirc->PutMULGate( s_a_w2[i][k] , s_a_c2in[k][j] );
            s_a_c2out[i][j] = acirc->PutADDGate( s_a_c2out[i][j] , s_a_tmp_0 );
        }
        s_a_c2out[i][j] = acirc->PutADDGate( s_a_c2out[i][j] , s_a_Rc2outIS48IE64[i][j] );
    }
}
uint32_t _tmp_2 = 1 ;
...
uint32_t _output = s_a_tmp_1->get_clear_value<uint32_t>();
