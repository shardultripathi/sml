...
auto Rc2outIS32IE48 = make_vector<uint32_t>(64, 1024);
auto s_a_Rc2outIS32IE48 = make_vector<share*>(64, 1024);
auto IRc2outIS32IE48 = make_vector<uint32_t>(64, 1024);
auto s_a_IRc2outIS32IE48 = make_vector<share*>(64, 1024);
if(role == SERVER){
ifstream fin("IRc2in");
while(!fin.eof()){
	uint32_t i, j, val;
...
for(int i = 32; i < 48; i++) {
	for(int j = 0; j < 1024; j++) {
		fout << i << " " << j << " " <<(IRc2outIS32IE48[i][j] = rand()) << endl;
	}
}
fout.close();
}
...
for (uint32_t i = 32; i < 48; i++)
{
    ...
    {
        if (role == SERVER) {
            Rc2outIS32IE48[i][j] = IRc2outIS32IE48[i][j] ;
            s_a_Rc2outIS32IE48[i][j] = acirc->PutINGate( Rc2outIS32IE48[i][j] ,bitlen,SERVER);
        } else {
            s_a_Rc2outIS32IE48[i][j] = acirc->PutDummyINGate(bitlen);
        }
    }
}
for (uint32_t i = 32; i < 48; i++)
...
for (uint32_t i = 32; i < 48; i++)
{
    for (uint32_t j = 0; j < 1024; j++)
    {
        ...
        s_a_c2out[i][j] = acirc->PutADDGate( s_a_c2out[i][j] , s_a_Rc2outIS32IE48[i][j] );
    }
}
uint32_t _tmp_3 = 1 ;
