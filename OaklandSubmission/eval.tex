...
We use $[N]$ to denote $\{0,1,\dotsc, N-1\}$. Further, given a vector $x\in\R^d$, we say $\mathsf{argmax}\ x = i$ if $x_i = \mathsf{max}\ \{x_1,\ldots,x_d\}$. We focus on the machine learning models for {\it classification}. A classifier $C$ uses a trained model to {\it predict} a
...
as we did earlier to be consistent?} \nc{I think I have fixed this. OK now? $\mathsf{argmax}$ throughout this section has implicit use of a dimension and I have left it like that. We can make it explicit if others feel the need.}
...
tree\footnote{We remark here that our current language does not support functions (which we leave for future work) and with this support, LOC would be even lower.}.\aseem{Should we remark here that once we have functions in the
...
on our machines \aseem{Is the limitation machine RAM or ABY
  limit?}\nc{ABY I think. But ABY's limitation is because of RAM I think}. Therefore, we use partitioning and divide the
...
succeed. \aseem{Last sentence is speculative? Should we say \minion
  would or could or might?}\nc{I think \minion definitely would, but we could say could to be nice.}
