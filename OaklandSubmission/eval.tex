...
of partitioning in \tool. Finally, we evaluate \tool on the task of matrix factorization~\cite{valeriaMatrix}. 
...
a classifier might be able to predict a label as ``will rain'' or ``will not rain''. The {\it
...
of matrix $w$, $\matmul$ denotes matrix multiplication (with a vector in this case) and $a>b$ returns $1$ if $a>b$ and $0$ otherwise. 
The {\it model size} of this classifier, i.e., the number of parameters in a model is $d$.
...
first row of Table~\ref{tab:tf}. We note that a pure boolean circuit for this functionality cannot fit in the memory of our machines.
