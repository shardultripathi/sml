...
for this example. The code first reads the server's (client's) input using
 \ls{input1} (\ls{input2}). It then uses a
...
(ABY). E.g., for the code in Figure~\ref{fig:ex-sml}, the \tool
...
representations require share-conversions. The \tool compiler also
...
guarantees. We provide a formal description of the \tool language next.
