...
Since its introduction in 1980s, there has been a long sequence
...
$w^{T}$ and $x$.  With the current state-of-the-art, Alice has following
options:
...
scalable \mpc protocols, comprising combinations of arithmetic and
boolean circuits, from high-level programs devoid of any
cryptography. Below we describe the salient features of \tool.

\subsubsection*{Programmability} \tool source programs are ideal
...
cryptographic details. For example, an \tool source
program for $w^Tx >b$ is as follows:
...
\vspace{0.2cm}
We note that the program is quite similar to what a programmer might
write in a common programming language. The simplicity of the
language comes with its usual benefits: it is easily accessible to the
developers, there are fewer avenues for making mistakes, and it is
easy to maintain and modify the programs. Needless to say, frameworks
that expose low-level circuit APIs to the programmer do not enjoy
these benefits.

\subsubsection*{Cryptogaphic-cost awareness} \tool compiler is aware
of the cryptographic costs of arithmetic and boolean representations
of the source language operators, and their corresponding
inter-conversion gates. Based on these costs, the compiler, given a
source program, automatically picks arithmetic or boolean
representations for different parts of the program, and generates the
corresponding circuits along with the required inter-conversion
gates. The outcome is an effcient \mpc protocol using a combination of
arithmetic and boolean circuits, while the programmer remains
oblivious of all the low-level cryptographic details. Our evaluation
shows that the performance of the
protocols automatically generated by the \tool compiler is comparable
to or better than the custom, specialized protocols developed in the
previous work. Indeed, \tool is the first such cryptographic-cost
aware compiler.
...
memory on a machine\footnote{Using swap and
...
prediction, etc. that operate on large amounts of data. \aseem{Can we
...
sub-programs, which are then sequentially processed by our
framework, while appropriately threading the intermediate outputs
along. While this
...
the security risk of revealing the intermediate outputs. To address
this, \tool (a) masks the intermediate outputs using a secure secret
sharing between the two parties, and (b) appends the
share-reconstruction function to the beginning of the subsequent
sub-program. As we show in our evaluation, partitioning allows us to
program large applications in \tool.

%% We provide details of partitioning and prove the
%% correctness and security of this transformation
%% in \sectionref{pipe}

\subsubsection*{Formal guarantees} We prove a formal correctness and
security theorem for our compiler. The correctness theorem relates the
...
compiled program. Both these semantics are instrumented
with \emph{observations} that record the outputs. We prove that a
well-typed source program terminates successfully in the source
semantics. In addition, we prove that for a well-typed source program,
its observations in the source semantics are same as the observations
of its corresponding compiled program in the protocol
semantics. {\aseem{Had a hard time writing this last sentence,
suggestions welcome.}}
For the security theorem, we formally reduce the security of our
compiler to the security of the back-end cryptographic
protocol.
...
\subsubsection*{Performance Baseline} Most works cite garbled
...
\aseem{Checkpointing here. Two more paragraphs to come, one for
summary of implementation and evaluation, and one for related work.}
...
We provide a detailed survey of related work in \sectionref{related}.
