...
protocols from previous work (up to a factor of 19x in our experiments, depending on the functionality) \nc{I added this but I guess we are saying it many times, but might be nice to highlight perf early no?}. Below we describe the
...
begin with an overview of \tool through an example in Section~\ref{sec:ex}. \nc{provided a pointer to sec 2 as well.}
