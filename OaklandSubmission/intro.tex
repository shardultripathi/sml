...
expert in cryptography, to easily program efficient and scalable \mpc
...
cryptographic details. For example, Figure~\ref{fig:introexample}
shows an \tool source
program for $w^Tx >b$.
%
%\vspace{0.2cm}
...
(e.g. \cite{barni,blanton,brickell,franz,huang,valeriaMatrix,valeriaRidge,schropferK11}),
  and some have also developed tools that allow writing such combinations
  (e.g. \cite{bogdanov,lone,tasty}). Kerschbaum et al.~\cite{autoS}
  even provide a scheme to automatically select optimal protocols
  for sub-compuations. However, as Demmler et al.~\cite{aby} note, due
...
protocol. Additionally, these prior works neither have a
developer-friendly language, nor do they formalize their type system
and operational semantics, and show them to be consistent.
