...
We have implemented the \tool toolchain using ABY~\cite{aby} as the
...
classifiers such as Tensorflow~\cite{tensorflow}
...
implementation of \bonsai~\cite{bonsai}, and it was programmed at a
high-level in \tool by a
cryptography novice. Second, the performance of the protocols
...
that require more than 100 million gates.

The rest of the paper is structured as follows. We formalize \tool and
prove its correctnss and security theorems in
Section~\ref{sec:ld}. Section~\ref{sec:pipe} details our partitioning
scheme and formally proves its security. Section~\ref{sec:impl}
and~\ref{sec:eval} present details of \tool implementation and
evaluation. Finally, Section~\ref{sec:related} talks about the related
work. We start with an overview of \tool.
