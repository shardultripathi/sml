...
state-of-the-art, let us consider the following example underlying many
...
inner product of  $w^{T}$ and $x$. With the current state-of-the-art,
Alice has following options:
...
protocols from previous work. Below we describe the
salient features of \tool.
...
public, and hence, the array accesses need not be compiled as
multiplexers. Further, within the secret parts, \tool compiler is
aware
...
forward simluation between the
...
theorem not only guarantees that a
well-typed source program terminates successfully in the ideal
semantics (e.g. there are no array index out-of-bounds errors), but
also that the observable outputs in the two semantics are the same. For
the security theorem, we formally reduce the security
of our compiler to the security of the back-end cryptographic
protocol. We also prove a formal security theorem for our partitioning
scheme.
