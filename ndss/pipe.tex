...
especially for ML applications,
can quite often be larger than the total memory
available on the machines executing the 2PC protocol. The secure
protocol for evaluating this circuit cannot then be generated without
swapping the circuit in and out of the main memory, thereby causing large slowdowns
(see Section \ref{sec:pipeeval}). To avoid this problem, we use
partitioning to
``split'' the high level functionality into smaller programs that
compile into small circuits that can completely fit
into the main memory. These small circuits are then evaluated
sequentially, with proper threading of the intermediate outputs, to
evaluate the entire program. Naturally, care must be
...
Concretely, let $\prog$ be a program in our source language that
...
execute.
%% Partitioning enables us to 
%% execute such programs via a source to source transformation that is
%% oblivious to the underlying \mpc backend.
Partitioning decomposes the
program $\prog$ into a sequence of smaller \tool programs
$\progn_1,\progn_2,\ldots,\progn_k$, as described below, such
...
However, unlike FastGC, we do not operate at a circuit level and the
scheme itself is independent of the specific
\mpc protocol. 
%
...
%}
We now describe the instrumentation to ensure the security of the
partitioning scheme.
...
Let $\prog$ takes (secret) inputs $x$ from Alice and
