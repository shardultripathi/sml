...
(line~\ref{line:dotproductloop}). We remark that, in our experiments, assigning public labels to control flow variables is critical for performance and/or security. However, we have observed that conservatively creating circuits for other public variables incurs minimal cost in practice.
...
\sectionref{impl}). At the expression level, \tool provides secret conditionals (using the '? :' operator) which are implemented using multiplexers, while secret dependent array accesses can also be implemented using the '? :' operator. We elaborate on cryptographic cost awareness in
\sectionref{ccc}.
...
Further implementation details can be found in~\sectionref{impl}.
