...
We show that it is critical to use a mix of arithmetic and boolean circuits  for performance.
 Previous work has observed that Yao's garbled circuits do not scale to machine learning examples that require a large number of multiplications~\cite{aby,tasty,autoS,secureml,minionn,chameleon}.
...
boolean parts. While finding an optimum split  is an NP-hard problem (the predicates in {\tt if} statements can have arbitrary non-linear arithmetic),
\tool uses heuristics based
on empirical micro-benchmarking that  perform well in practice (Section~\ref{sec:eval}).
...
circuit for addition. The size of this circuit grows  linearly in the bitwidth of the inputs. Similarly, converting from boolean to arithmetic requires computing a 
...
communication in a LAN setting). Theoretically, the size of a boolean multiplication circuit is quadratic in the bitwidth, which causes this performance gap.
