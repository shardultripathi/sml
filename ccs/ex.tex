...
details. The language provides multi-dimensional arrays \dg{with public indices}, conditional
expressions (the ternary $?\::$ operator), \ls{for}~loops,
\ls{if}~statements, and special syntax for input/output from each party.
\TODO{Add a justification for not having secret indices, public inputs etc. Unless there is a better place to have those.}
...
\dg{Speaking loosely,} multiplying two $64$-bit integers using arithmetic sharing requires only $2$ multiplications, whereas Yao requires at least $4096$ AES operations in the online phase.
...
\label{sec:ccc}
\dg{Points to address. 1. hard problem to solve optimally 2. some decisions are fixed and some are context sensitive 3. our heuristics and their basis}
One of the main challenges for our compiler is to split the compute between arithmetic and boolean parts well. 
As mentioned above, \tool is a cryptographic cost-aware compiler and uses the arithmetic and boolean costs of primitive operators and the inter-conversion costs to make the split. We analyse these costs by running microbenchmarks for basic operations and interconversions and our observations were consistent with ABY~\cite{aby} (see Figures 2 and 3 in \cite{aby}). Based on these costs, our compiler has some hard constraints. For example, multiplication should always be performed in arithmetic circuit as the cost of boolean multiplication is much higher than  the cost of coercing from boolean to arithmetic, performing an arithmetic multiplication, and then coercing from arithmetic to boolean \dg{($\sim9x$ time $\&$ $\sim20x$ comm. in LAN setting)}.
...
We note that doing an optimal split between arithmetic and boolean is an \dg{NP-hard} problem and in this work, we use some heuristics based on costs observed using microbenchmarking. Moreover, for our machine learning benchmarks (such as linear classifier, DNNs, etc) these choices seem optimal.

\dg{\textsc{People wanted estimate of interconversion costs. I don't know where to put it. May be implementation?} Interconversion costs: Converting from A to Y requires computing computing a Yao circuit for addition that is linear size and hence, requires linear number of symmetric key operations. Similarly, Y2A requires computing a subtraction circuit that is again linear size.}
