...
\sectionref{impl}). We elaborate on cryptographic cost awareness in
\sectionref{ccc}.
...
input to the comparison and multiplexer circuits. \aseem{Question: why
  is b arithmetic to begin with? Can this input be boolean and then we
  don't have to insert a conversion gate for it?}
%\vspace{-0.2in}
...
\aseem{We decided to put it here, but reading now, this subsection
  seems to be a bit misplaced to me. How about moving it to 6.1, the
  first subsection in the
  Evaluation? And we can add a couple of lines about it in the
  introduction evaluation paragraph (We first show that mixed
  protocols are much more efficient than Yao only by comparing with
  blah blah). Here it seems quite abrupt, we
  are talking about EzPC at a high-level, with compiler features
  etc. and suddenly details about numbers, CIFAR and what not start
  coming. Seems too soon.}
...
Indeed, this is one of the main drivers behind development of various specialized MPC protocols in previous works \cite{secureml,minionn,shafindss} that we compare against in \sectionref{eval}.

The largest example benchmark in CBMC-GC is multiplication of two $8\times 8$ matrices for which it generates about a million gates and takes about ten seconds to run. In contrast, in \tool we use arithmetic sharing for this compute that generates 1218 gates and runs in about 0.1 seconds. We tried running CBMC-GC for multiplying two $20\times 20$ matrices and it timed out after 5 hours. In contrast, most of our benchmarks are much bigger. For example, we run the secure computation of a DNN for CIFAR-10 in 265 seconds which requires multiplying a $64\times576$ matrix with a $576\times1024$ matrix as a sub-computation. On the other hand, ObliVM scales to larger benchmarks with its use of Yao-pipelining \cite{yao-pipe}. We ran inner-product computations of varying lengths followed by a comparison in both ObliVM and \tool (see Table~\cite{oblivm-compare} in the Appendix). 
Our experiments show that we are atleast 30x faster than ObliVM due to our use of arithmetic sharing for inner-product computation. 
...
the secret computation in a program between arithmetic and
boolean. While an optimal split in general is an NP-hard problem
\aseem{can we cite something?}, \tool heuristics are empirically based
on micro-benchmarking, and perform well on the various secure
prediction benchmarks (Section~\ref{sec:eval}).

\paragraph{Micro-benchmarking.} The cost of splitting the compuation
between arithmetic and boolean involves the cost of operations
(addition, multiplication, etc.) and the cost of inter-conversion
gates between arithmetic and boolean. Cryptographic cost of converting
an arithmetic share to a boolean share requires computing a garbled
circuit for addition that is linear in size. Similarly, the cost of
converting from boolean to arithmetic is computing a subtraction
circuit inside garbled circuit that is again linear in size. Demmler
et al.~\cite{aby} analyze all these costs by running
microbenchmarks for basic operations and interconversions (Figures
2 and 3 in \cite{aby}). \tool heuristics are based on these results.

\paragraph{Multiplication.} For multiplication, \tool always chooses
arithmetic circuit, as the cost of boolean multiplication is much
higher than the cost of coercing the operands from boolean to
arithmetic, performing an arithmetic multiplication, and then coercing
the result back to boolean ($\sim9x$ more time $\&$ $\sim20x$ more
communication in a LAN setting). Since the performance gap is quite
large, we believe that this choice is optimal for realistic network
settings.

\paragraph{Addition.} However, this is not the case for addition. If
both the operands of addition are boolean shared, \tool performs
addition using a boolean circuit, else it uses an arithmetic
circuit. \aseem{What if one operand is in boolean and the other is in
  arithmetic?} Hence, addition may be done in arithmetic or boolean
depending on the operands.

\paragraph{Boolean operations.} Finally, \tool chooses Yao for for all
the boolean operations such as comparison, shifts, multiplexors,
etc. \aseem{Can we write a sentence here that Demmler et al. show this
