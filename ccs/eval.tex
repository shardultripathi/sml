...
succeed. For example, consider a large DNN with $n$ layers such that the circuit for only a single layer can  fit into memory. Then, \tool using partitioning can evaluate this large DNN in $n$ stages. To further illustrate the need for and utility of partitioning, we ran a series of DNNs with different layers with and without partitioning (where partitioning was done for every layer of the DNN). For up to 5-layer DNNs, the performance of both settings was comparable, thereby illustrating that partitioning did not cause any overhead to performance. For networks that had 6 or more layers, performance of the protocol without partitioning dramatically dropped, while the protocol with partitioning continued to scale linearly. These results are illustrated in Figure~\ref{fig:partitioning}.
...
Subsequent to our work, there have been improvements in the cryptographic backends.
For example, Gazelle describes specialized protocols for DNNs---which use a 
lattice-based packed additively homomorphic encryption scheme (PAHE) for arithmetic computations and garbled circuits for boolean computations---that can evaluate the CNN benchmark of Table~\ref{tab:nn} in 0.8 seconds (as opposed to
5.1 seconds taken by \tool when using ABY). Such advances in cryptography are
orthogonal to our contributions. In particular, we are interested in using Gazelle
as our backend when it becomes available. Furthermore, the authors of Gazelle remark:
...
\tool addresses this exact problem of factoring computations into
arithmetic and boolean parts.
