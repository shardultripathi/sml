...
succeed. For example, consider a large DNN with $n$ layers such that the circuit for only a single layer can  fit into memory. Then, \tool using partitioning can evaluate this large DNN in $n$ stages.
