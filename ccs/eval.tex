...
succeed. For example, consider a large DNN with $n$ layers such that
the circuit for only a single layer can  fit into memory. Then, \tool
using partitioning can evaluate this large DNN in $n$ stages. To
further illustrate the need for and utility of partitioning, we ran a
series of DNNs with different layers with and without partitioning
(where partitioning was done for every layer of the DNN). For up to
5-layer DNNs, the performance of both settings was comparable, thereby
illustrating that partitioning did not cause any overhead to
performance. For networks that had 6 or more layers, performance of
the protocol without partitioning dramatically dropped, while the
protocol with partitioning continued to scale linearly. These results
are illustrated in Figure~\ref{fig:partitioning}.
...
Subsequent to our work, \aseem{TODO} et al. \aseem{cite} have
presented Gazelle, a framework for specialized DNN
protocols. Gazelle use a lattice-based packed additively homomorphic
encryption scheme (PAHE) for arithmetic computations and garbled
circuits for boolean computations. Gazelle can evaluate the CNN benchmark of
...
particular, once Gazelle is available, we could add it as another
